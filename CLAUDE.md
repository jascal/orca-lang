...
**Future: Numeric Range Conditions in Decision Tables** ⏳ Not started: The loan demo (`demo-go/orca/loan-workflow.orca.md`) uses informal DTs with range-based cell values (`750+`, `<600`, `700-749`, `0.3-0.4`) because the formal DT syntax only supports discrete types (enum, bool). This is a **parser/verifier implementation gap, not a fundamental limitation** — numeric ranges are well-defined in decision table theory (cf. DMN/FEEL standard). What needs to change:
- **`dt-ast.ts`**: Extend `CellValue` with `{ kind: 'compare'; op: '>' | '>=' | '<' | '<='; value: number }` and `{ kind: 'range'; low: number; high: number; lowInc: boolean; highInc: boolean }` kinds. Add `decimal_range` to `ConditionType`.
- **`dt-parser.ts`**: `parseCellValue` recognizes `750+`, `>=750`, `<600`, `700..749`, `0.3-0.4` patterns. Currently these fall through to `{ kind: 'exact', value: '750+' }` — stored as strings but never evaluated correctly.
- **`dt-verifier.ts`**: `cellMatches` handles numeric comparisons; completeness checking switches from combination enumeration to interval arithmetic (union of rule intervals must cover the declared domain). `DT_COVERAGE_GAP` also needs an interval-based path for numeric fields since the current approach enumerates combinations, which doesn't work for infinite domains.
- **`dt-compiler.ts`**: Emit `>=`/`<=`/range comparisons instead of `===` for numeric conditions in generated TypeScript, Python, and Go code.
- The `int_range` condition type stub already exists in `ConditionDef` (with `range?: { min, max }`) but is explicitly skipped in completeness checking with `DT_COMPLETENESS_SKIPPED` — it was scaffolded but never implemented.
